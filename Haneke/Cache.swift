...
public class Cache<T : DataLiteralConvertable> {
    ...
    public func fetch(key key : String, @autoclosure(escaping) value getValue : () -> T, formatName : String = HanekeGlobals.Cache.OriginalFormatName, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
